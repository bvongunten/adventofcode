package ch.nostromo.adventofcode.year2023.day15;
...
public class Part2 extends BasePuzzle {
    ...
    private static final String EXPECTED_TEST_RESULT = "145";
    ...
    public String solvePuzzle(List<String> input) {
        ...
        for (String str : input.get(0).split(",")) {
            if (str.contains("=")) {
                ...
                int box = hash(label);
                int fl = Integer.valueOf(str.split("=")[1]);

                boxes.get(box).replaceOrAdd(new FocalLength(label, fl));
            } else {
                String label = str.split("-")[0];
                int box = hash(label);

                boxes.get(box).remove(new FocalLength(label, -1));
            }
        }

        for (int i = 0; i < boxes.size(); i++) {
            result += boxes.get(i).getScore(i);
        }
    ...
    @Data
    public class Box {
        List<FocalLength> focalLengths = new ArrayList<>();

        public void remove(FocalLength fl) {
            focalLengths.remove(fl);
        }

        public void replaceOrAdd(FocalLength fl) {
            if (focalLengths.contains(fl)) {
                focalLengths.get(focalLengths.indexOf(fl)).setValue(fl.getValue());
            } else {
                focalLengths.add(fl);
            }
        }

        public int getScore(int boxId) {
            int result = 0;
            for (int i = 0; i < focalLengths.size(); i++) {
                result += (boxId + 1) * (i + 1) * focalLengths.get(i).value;
            }
            return result;
        }
    ...
    public class FocalLength {
        ...
        public FocalLength(String label, int value) {
            this.label = label;
            this.value = value;
        }
