...
public class Part2 extends BasePuzzle {
    ...
    private static final String EXPECTED_TEST_RESULT = "2858";
    ...
    List<Block> parseDiskMap(String input) {
        ...
        for (char ch : input.toCharArray()) {
            ...
            if (size > 0) {
                ...
                result.add(new Block(fileId, isFile, size));
            }
            if (isFile) {
                fileId++;
            }
    ...
    void compactDisk(List<Block> blocks) {
        ...
        while (readIndex >= 0) {
            ...
            if (current.isFile) {
                int freeBlockidx = getFirstFreeSpot(blocks, current.size);

                if (freeBlockidx >= 0 && freeBlockidx < readIndex) {
                    ...
                    int sizeDiff = toReplace.size - current.size;

                    // insert new Block if it was smaller, update idx
                    if (sizeDiff > 0) {
                        blocks.add(freeBlockidx + 1, new Block(-1, false, sizeDiff));
                        toReplace.size = toReplace.size - sizeDiff;
                        readIndex++;
                    }
                }
    ...
    int getFirstFreeSpot(List<Block> blocks, int size) {
        for (int i = 0; i < blocks.size() - 1; i++) {
            if (!blocks.get(i).isFile && blocks.get(i).size >= size) {
                return i;
            }
        }
        return -1;
        ...
        for (Block block : blocks) {
            ...
            if (block.isFile) {
                for (int i = 0; i < block.size; i++) {
                    checksum += (long) posIdx * block.fileId;
                    posIdx++;
                }
            } else {
                posIdx += block.size;
