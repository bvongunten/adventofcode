...
public class Part2 extends BasePuzzle {
    ...
    private static final String WENN_ICH_GROSS_BIN_WILL_ICH_SEIN_WIE_OSI_UND_ROLE = "9";
    ...
    private  int countOccurrences(char[][] grid) {
        ...
        for (int row = 1; row < rows - 1; row++) {
            for (int col = 1; col < cols - 1; col++) {
                if (grid[row][col] == 'A' &&
                        isMasOrSam(grid, row - 1, col - 1, row + 1, col + 1) &&
                        isMasOrSam(grid, row - 1, col + 1, row + 1, col - 1)) {
                    result++;
                }
            }
    ...
    private boolean isMasOrSam(char[][] grid, int row1, int col1, int row2, int col2) {
        String word = grid[row1][col1] + "A" + grid[row2][col2];
